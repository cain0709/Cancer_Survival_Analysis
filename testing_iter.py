...
if __name__ == '__main__':
	...
	df.replace({'N': 0, 'Y': 1,  'M' : 0, 'F' : 1}, inplace = True)
	...
	df_targets_train =  df_train[['duration','dead']]
	df_targets_val =  df_val[['duration','dead']]

	print(df_targets_train.shape)
	print(df_targets_val.shape)
	print(df_targets_train)
	print(df_targets_val)

	df_imp_train = df_train.drop(['duration','dead'],axis=1)
	df_imp_val = df_val.drop(['duration','dead'],axis=1)
	cols = df_imp_train.columns

	df_imp_train = scaler.fit_transform(df_imp_train)
	df_imp_val = scaler.transform(df_imp_val)



	#preprocess
	imputer = KNNImputer(n_neighbors=5)
	df_imp_train = imputer.fit_transform(df_imp_train)
	df_imp_val = imputer.transform(df_imp_val)

	df_imp_train = pd.DataFrame(df_imp_train, columns=cols)
	df_imp_val = pd.DataFrame(df_imp_val, columns=cols)

	print(df_imp_train.isnull().sum())
	print(df_imp_val.isnull().sum())

	print(df_targets_train.shape)
	print(df_targets_val.shape)
	print(df_imp_train.shape)
	print(df_imp_val.shape)

	df_imp_train = pd.concat([df_imp_train, df_targets_train],axis=1)
	df_imp_val = pd.concat([df_imp_val, df_targets_val],axis=1)

	print(df_imp_train)
	print(df_imp_val)

	df_imp_train = df_imp_train.astype('float32')
	df_imp_val = df_imp_val.astype('float32')

	get_target = lambda df: (df['duration'].values, df['dead'].values)



	train_y = get_target(df_imp_train)
	val_y = get_target(df_imp_val)

	df_imp_train.drop(['duration','dead'],axis=1,inplace=True)
	df_imp_val.drop(['duration','dead'],axis=1,inplace=True)

	train_x = df_imp_train
	val_x = df_imp_val
